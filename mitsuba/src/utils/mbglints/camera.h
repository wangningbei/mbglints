...
public:
	Camera(const Vec3 &_pos, const Vec3 &_dir, const Vec3 &_up, const Vec3 &_left, const Vec2i &_res, float _fov,
		...
		cout << "m_s:" << m_s[0] << ", " << m_s[1] << "," << m_s[2] << endl;
		cout << "m_t:" << m_t[0] << ", " << m_t[1] << "," << m_t[2] << endl;
		...
		cout << "M_A1: " << m_resolution[0] * m_resolution[1] / m_A << endl;
	...
	bool  computeDifferentialAutoDiff(const Path &path, Vec3 &out_dDdx, Vec3 &out_dDdy, Vec3 &out_dPdx, Vec3 &out_dPdy)
	{
		const int pathLength = path.getLength() - 2;
	...
	void splat(const Path *path)
	{
		...
		if (pixel[0] < m_resolution[0] && pixel[0] >= 0 && pixel[1] < m_resolution[1] && pixel[1] >= 0)
		{
			...
			bool valid = computeDifferentialAutoDiff(*path, dDdx, dDdy, dPdx, dPdy);
			if (!valid)
				return;

			float temp = dPdx.norm() * dPdy.norm() * std::max(0.1f, dPdx.normalized().cross(dPdy.normalized()).norm());
			float curvatureFac = 1.0 / temp;
			Vec3 value = path->computeContribution() * curvatureFac * m_resolution[0] * m_resolution[1] / m_A;
	...
	void splat(const std::vector<Path*> &pathList, const Scene *scene)
	{
		const int threadCount = omp_get_max_threads();
